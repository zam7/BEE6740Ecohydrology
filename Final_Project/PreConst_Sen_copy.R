...
####################################################################################
# Subset MetData to account for the correct period of analysis
num_year = 4
MetDataSub = data.frame(matrix(nrow = 365*num_year+1, ncol = 0))

for (i in 1:length(MetData$Year))
{
  if (MetData$Year[i] > 1989 && MetData$Year[i] < 1994)
  {
   MetDataSub$Year[i] = MetData$Year[i]
   MetDataSub$Precip_mm[i] = MetData$Precip_mm[i]
   MetDataSub$Month[i] = MetData$Month[i]
   MetDataSub$Day[i] = MetData$Day[i]
   MetDataSub$Tmax_C[i] = MetData$Tmax_C[i]
   MetDataSub$Tmin_C[i] = MetData$Tmin_C[i]
   MetDataSub$Date[i] = MetData$Date[i]
  }
}
####################################################################################

#Step 2: Calculate average daily temperature for each record
MetDataSub$Tavg_C = (MetDataSub$Tmax_C + MetDataSub$Tmin_C)/2
...
###### Analyze period before construction of mall (1994-1997) 
SnowMelt_pre = SnowMelt(MetDataSub$Date, MetDataSub$Precip_mm, MetDataSub$Tmax_C, MetDataSub$Tmin_C, lat_rad_WN)
...
for (i in 1:n_runs)
{
  snow = SnowMelt(MetDataSub$Date[1:(365*4)], MetDataSub$Precip_mm[1:(365*4)], MetDataSub$Tmax_C[1:(365*4)], 
                  MetDataSub$Tmin_C[1:(365*4)], lat_rad_WN, windSp = u_rand[i], forest = fc_rand[i])
...
AllData = data.frame(matrix(nrow = length(MetDataSub$Year), ncol = 0))
