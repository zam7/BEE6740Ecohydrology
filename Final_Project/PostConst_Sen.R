...
MetData <- read.csv("final_post_metdata.csv")
...
###### Analyze period after construction of mall (1997-1998) 
####################################################################################
SnowMelt_pre = SnowMelt(MetData$Date[1:(365*1.5)], MetData$Precip_mm[1:(365*1.5)], MetData$Tmax_C[1:(365*1.5)], MetData$Tmin_C[1:(365*1.5)], lat_rad_WN)
####################################################################################
...
{
  ####################################################################################
  snow = SnowMelt(MetData$Date[1:(365*1.5)], MetData$Precip_mm[1:(365*1.5)], MetData$Tmax_C[1:(365*1.5)], 
                  MetData$Tmin_C[1:(365*1.5)], lat_rad_WN, windSp = u_rand[i], forest = fc_rand[i])
  ####################################################################################
...
NS_HSR_obs = get_usgs_gage(flowgage_id = "01376800", begin_date = "1997-01-01", end_date="1998-10-30")
####################################################################################
...
#################################################################################### changes for each condition
AllData = data.frame(matrix(nrow = 547, ncol = 0))
####################################################################################
AllData$sim_obs = NS_HSR_sim$modeled_flow[1:547] - NS_HSR_obs$flowrate_mmperd[1:547]
NSE_numerator = sum(AllData$sim_obs[1:547] * AllData$sim_obs[1:547])
AllData$obs_obs = NS_HSR_obs$flowrate_mmperd[1:547] - mean(NS_HSR_obs$flowrate_mmperd[1:547])
NSE_denom = sum(AllData$obs_obs[1:547] * AllData$obs_obs[1:547])
